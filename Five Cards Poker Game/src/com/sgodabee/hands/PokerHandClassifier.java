...
public class PokerHandClassifier implements HandClassifier {
    ...
    private final CategoryGroup rankGroup;
    private final TypeGroup suitGroup;
    private final SortedSet<Card> cards;

    PokerHandClassifier(final CategoryGroup rankGroup,
    ...
    private Classification isPair() {
        if (this.rankGroup.getDoubleCount() == 1) {
            final Iterator<Map.Entry<Categories, List<Card>>> rankGroup = this.rankGroup.iterator();
    ...
    private Classification detectTwoPair() {
        if (this.rankGroup.getDoubleCount() == 2) {
            final Iterator<Map.Entry<Categories, List<Card>>> rankGroup = this.rankGroup.iterator();
            final SortedSet<Card> cards = new TreeSet<>();
            cards.addAll(rankGroup.next().getValue());
            cards.addAll(rankGroup.next().getValue());
    ...
    private Classification detectNormalStraight() {
        final Set<Categories> cardRanks = this.rankGroup.getCategoriesMap().keySet();
    ...
    private SortedSet<Card> calculateStraight(final List<Categories> ranks) {
        final SortedSet<Card> results = new TreeSet<>();
        final Map<Categories, List<Card>> rankGroup = this.rankGroup.getCategoriesMap();
        for (final Categories rank : ranks) {
    ...
    private Classification detectWheel() {
        final List<Categories> wheelRanks = Arrays.asList(Categories.ACE, Categories.TWO, Categories.THREE, Categories.FOUR, Categories.FIVE);
        final Set<Categories> handRanks = new TreeSet<>(this.rankGroup.getCategoriesMap().keySet());
        handRanks.retainAll(wheelRanks);
        if (handRanks.containsAll(wheelRanks)) {
            final SortedSet<Card> cards = new TreeSet<>();
            for (final Map.Entry<Categories, List<Card>> entry : this.rankGroup.getCategoriesMap().entrySet()) {
    ...
    private Classification detectFlush() {
        final Map<Types, List<Card>> suitGroup = this.suitGroup.getTypesMap();
        for (final Map.Entry<Types, List<Card>> entry : suitGroup.entrySet()) {
    ...
    private Classification detectFullHouse() {
        if (this.rankGroup.getSetCount() == 2 ||
            (this.rankGroup.getSetCount() == 1 && this.rankGroup.getDoubleCount() >= 1)) {
            final Iterator<Map.Entry<Categories, List<Card>>> handRankIterator = this.rankGroup.iterator();
            final SortedSet<Card> cards = new TreeSet<>();
    ...
    private static Collection<Card> extractFullHousePair(final Iterator<Map.Entry<Categories, List<Card>>> handRankIterator) {
    ...
    private static Card extractQuadKicker(final Iterator<Map.Entry<Categories, List<Card>>> rankGroup) {
        ...
        for (final Map.Entry<Types, List<Card>> entry : suitGroup.entrySet()) {
            if (entry.getValue().size() == 5) {
                ...
                for (int i = 0; i < cardArray.length - 1; i++) {
                    if (cardArray[i].getCategories().getCategories() != cardArray[i + 1].getCategories().getCategories() - 1) {
